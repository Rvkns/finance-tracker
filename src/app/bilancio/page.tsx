...
export default async function BilancioPage() {
    ...
    // Fetch ALL transactions sorted oldest first for timeline
    ...
    // Build month rows with running cumulative balance
    let runningBalance = 0; // positive = partner owes me, negative = I owe partner
    const months = sortedKeys.map(key => {
        ...
        const monthDelta = myPaid - myFairShare; // + partner owes me this month, - I owe partner
        runningBalance += monthDelta;
        return { key, myPaid, partnerPaid, total, monthDelta, runningBalance };
    });

    // Cumulative overall numbers
                ...
                years.map(year => (
                    <section key={year} className={styles.yearSection}>
                        ...
                        <div className={styles.timeline}>
                            {byYear[year].slice().reverse().map((m, i) => {
                                ...
                                return (
                                    <div key={m.key} className={`${styles.monthCard} ${isNow ? styles.monthCardCurrent : ''}`}>
                                        <div className={styles.monthCardHeader}>
                                            ...
                                            {/* Running cumulative at end of this month */}
